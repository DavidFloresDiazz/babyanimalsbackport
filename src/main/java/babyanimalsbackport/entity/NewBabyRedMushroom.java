...
public class NewBabyRedMushroom extends MushroomCow {
    ...
    public NewBabyRedMushroom(EntityType<? extends MushroomCow> type, Level level) {
        super(type, level);
    }
    ...
    public void tick() {
        ...
        if (this.level() == null) {
            System.err.println("[ERROR] NewBabyRedMushroom.tick() - level() es null!");
            return;
        }
        ...
        if (!this.level().isClientSide && this.isBaby()) {
            ...
            if (ageTicks % 100 == 0) {
                System.out.println("[NewBabyRedMushroom] Age: " + ageTicks + " / " + GROWTH_TICKS);
            }

            if (ageTicks >= GROWTH_TICKS) {
                System.out.println("[NewBabyRedMushroom] ¡CONVERTIENDO A MOOSHROOM ADULTO!");
    ...
    private void convertToAdultMushroom() {
        if (this.level() == null) {
            System.err.println("[ERROR] NewBabyRedMushroom.convertToAdultMushroom() - level() es null!");
            return;
        }
